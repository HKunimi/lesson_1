...
### 1.2 データベース設計とセットアップ
- [x] Supabaseプロジェクトの初期化（ユーザーがSupabaseダッシュボードで実施）
- [x] データベーススキーマの作成
  - [x] usersテーブル（ClerkのユーザーIDをPKとして使用・初回ログイン時にupsert）
  - [x] categoriesテーブル（カテゴリ管理）
  - [x] time_entriesテーブル（作業時間記録）
- [x] マイグレーションファイルの作成（supabase/migrations/）
- [x] Row Level Security (RLS)ポリシーの設定
- [x] ClerkとSupabaseの連携設定（カスタムヘッダー方式）
 
### 1.3 認証システムの実装
- [x] Clerk Providerの設定（app/providers.tsx）
- [x] 認証用Middlewareの設定（proxy.ts ※Next.js 16はmiddleware.tsではなくproxy.ts）
- [x] サインイン/サインアップページの作成
  - [x] /sign-in ページ
  - [x] /sign-up ページ
- [x] ユーザーメニューコンポーネントの作成（Header.tsx内のUserButtonで実装済み）
- [x] 認証状態の動作確認
 
## フェーズ2: 基本機能の実装
 
### 2.1 レイアウトとナビゲーション
- [x] 共通レイアウトの作成（app/(main)/layout.tsx）
- [x] ヘッダーコンポーネントの実装
  - [x] ロゴ
  - [x] ナビゲーションメニュー
...
- [x] 手動入力フォームの作成
  - [x] 日付選択
  - [x] 作業時間直接入力（2モード：開始〜終了時刻 / 時間・分直接入力）
  - [x] カテゴリ選択（必須）
  - [x] メモ入力（オプション）
- [x] フォームバリデーション
- [x] 入力確認と保存処理
...
- [x] 作業記録のCRUD API Routes
  ...
  - [x] GET /api/time-entries（一覧取得・日付フィルタ・limit対応）
...
### 4.1 作業履歴表示
- [x] 作業履歴一覧コンポーネント（components/WorkHistory.tsx）
  - [x] 日付ごとのグルーピング（日付ヘッダー + その日の合計時間）
  - [x] カテゴリ表示（色付きドット + カテゴリ名）
  - [x] 作業時間の表示（HH:MM - HH:MM 時間帯 + 合計時間）
  - [x] メモの表示
  - [x] 編集モーダル（日付・開始/終了時刻・カテゴリ・メモ）
  - [x] 削除確認モーダル
 
### 4.2 基本的な統計表示（無料版）
- [x] 合計時間の計算と表示
  - [x] 今日の合計（件数）
  - [x] 今週の合計（件数・アクティブ日数・日平均）
  - [x] 今月の合計（件数・アクティブ日数・日平均）
  - [x] 今週の日別バーチャート（月〜日、今日をハイライト）
 
### 4.3 データエクスポート機能
- [x] CSV形式でのエクスポート（BOM付きUTF-8、Excel対応）
- [x] エクスポート範囲の選択（今日/今週/今月/過去30日/過去90日/カスタム）
- [x] ダウンロード処理の実装（GET /api/export）
 
## フェーズ5: プレミアム機能の実装
 
### 5.1 Clerk Billing設定
- [x] 料金ページの作成（/pricing）
- [x] PricingTableコンポーネントの実装（Clerk準拠）
- [x] プラン比較表の作成
 
### 5.2 アクセス制御
- [x] PlanProtectコンポーネントの実装（Clerkの組み込みコンポーネント）
- [x] プレミアム機能へのアクセス制限
- [x] アップグレード促進UI
